...
static const DefaultMacro internal_macros[] = {
	...
	{"pg_catalog", "map_to_pg_oid", {"type_name", nullptr}, {{nullptr, nullptr}}, "case type_name when 'bool' then 16 when 'int16' then 21 when 'int' then 23 when 'bigint' then 20 when 'date' then 1082 when 'time' then 1083 when 'datetime' then 1114 when 'dec' then 1700 when 'float' then 700 when 'double' then 701 when 'bpchar' then 1043 when 'binary' then 17 when 'interval' then 1186 when 'timestamptz' then 1184 when 'timetz' then 1266 when 'bit' then 1560 when 'guid' then 2950 when 'any' then 2276 when 'varchar' then 1043 else null end"}, // map duckdb_oid to pg_oid. If no corresponding type, return null
	...
	{DEFAULT_SCHEMA, "array_upper", {"arr", "dim", nullptr}, {{nullptr, nullptr}}, "array_length(arr, dim)"},
